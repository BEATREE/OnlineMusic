  ...
  <body>
    	...
    	<div class="container">
			...
			<div class="leftPart">
				<div class="row listDesc">
					...
					<div class="col-sm-8">
						<h5>关于&nbsp;<span style="color:red;">${keywords }</span>&nbsp;的搜索结果</h5>
				...
				<div class="bigPart">
					...
					<table class="table table-hover table-striped">
						...
						<tbody>
							<s:iterator value="#session.resultSongs" id="ressl" status="L">
								<tr><td>${L.index+1}</td><td>${ressl.getSongname() }</td><td>${ressl.getSongsinger() }</td><td>${hotsl.getSongplaynum() }</td>
								<td>${ressl.getUser().getNickname()}</td><td><a href="DetailSongs?sid=${ressl.getSid() }" class="btn btn-sm btn-success" target="_blank"><i class="fui-bookmark"></i>&nbsp;查看</a>
									<a href="${ressl.getSongsrc() }" class="btn btn-sm btn-success" target="_blank"><i class="fui-triangle-down"></i>&nbsp;下载</a></td>
								</tr>
							</s:iterator>
						</tbody>
					</table>
	  ...
	  <script src="http://www.jq22.com/jquery/jquery-2.1.1.js"></script>
		...
		var player = new MPlayer({
			...
			lrcTopPos: 34,
			// 列表模板，用${变量名}$插入模板变量
			listFormat: '<s:iterator value="#session.resultSongs" id="hotsl"><tr><td>${hotsl.getSongname()}</td><td>${hotsl.getSongsinger()}</td><td>见控制条时长</td></tr></s:iterator>',
			// 音量滑块改变事件名称
