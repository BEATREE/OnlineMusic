...
<html>
  <head>
    ...
    <title>${session.singerName}演唱的歌曲 | 梯云音乐</title>
    	...
    	<div class="container">
			...
			<div class="leftPart">
				<div class="row listDesc">
					<div class="col-sm-4">
						<img class="repImg" src="img/yuanchuangbang.png"/>
					</div>
					<div class="col-sm-8">
						<h5>歌手：&nbsp; ${session.singerName} </h5>
						介绍：
						<p>	
						共有歌曲<b>${session.singerSongs.size() }</b>首
						</p>
					</div>
				</div>
				<div class="bigPart">
					...
					<table class="table table-hover table-striped">
						...
						<tbody>
							<s:iterator value="#session.singerSongs" id="singerSong" status="ss">
								<tr><td>${ss.index+1}</td><td>${singerSong.getSongname() }</td><td>${singerSong.getSongsinger() }</td><td>${singerSong.getSongplaynum() }</td>
								<td>${singerSong.getUser().getNickname()}</td><td><a href="DetailSongs?sid=${singerSong.getSid() }" class="btn btn-sm btn-success" target="_blank"><i class="fui-bookmark"></i>&nbsp;查看</a>
									<a href="${singerSong.getSongsrc() }" class="btn btn-sm btn-success" target="_blank"><i class="fui-triangle-down"></i>&nbsp;下载</a></td>
								</tr>
							</s:iterator>
						</tbody>
					</table>
