...
<html>
  <head>
    ...
    <title>${session.currentPlaylist.getPlname() }-歌单 | 梯云音乐</title>
  ...
  <body>
    	...
    	<div class="container">
			...
			<div class="leftPart">
				<div class="row listDesc">
					<div class="col-sm-4">
						<img class="repImg" src="${session.currentPlaylist.getPlimg() }"/>
					</div>
					<div class="col-sm-8">
						<h5>${session.currentPlaylist.getPlname() }</h5>
						播放次数：${session.currentPlaylist.getPlnumber() }<br>
						介绍：
						<p>	
						${session.currentPlaylist.getPldesc() }
						</p>
					</div>
				</div>
				<div class="bigPart">
					...
					<table class="table table-hover table-striped">
						...
						<tbody>
							<s:iterator value="#session.currentPlaylist.getPlsongs()" id="plsong" status="step">
								<tr><td>${step.index+1 }</td><td>${plsong.getSong().getSongname() }</td><td>${plsong.getSong().getSongsinger() }</td>
							<td>${plsong.getSong().getSongplaynum() }</td><td>${plsong.getSong().getUser().getNickname() }</td>
							<td><a href="DetailSongs?sid=${plsong.getSong().getSid() }" class="btn btn-sm btn-success" target="_blank"><i class="fui-bookmark"></i>&nbsp;查看</a>
							<a href="${plsong.getSong().getSongsrc() }" class="btn btn-sm btn-success" target="_blank"><i class="fui-triangle-down"></i>&nbsp;下载</a></td>
							</tr>
							</s:iterator>
						</tbody>
					</table>
				...
				<div class="hotList">
					...
					<ul>
						<s:iterator value="#session.hotSongList" id="hsl">
							<a><li>${hsl.getSongname() }&nbsp;————&nbsp;${hsl.getSongsinger() }</li></a>
						</s:iterator>
					</ul>
				</div>
			</div>
	  ...
	  <script src="http://www.jq22.com/jquery/jquery-2.1.1.js"></script>
		...
		var player = new MPlayer({
			...
			listFormat: '<s:iterator value="#session.currentPlaylist.getPlsongs()" id="plsong" status="step"><tr><td>${plsong.getSong().getSongname() }</td><td>${plsong.getSong().getSongsinger() }</td><td>见控制条时长</td></tr></s:iterator>',
			// 音量滑块改变事件名称
