...
<html>
  <head>
    ...
    <title>${session.currentSong.getSongname() }-歌曲 | 梯云音乐</title>
    
	<meta http-equiv="pragma" content="no-cache">
	<meta http-equiv="cache-control" content="no-cache">
	<meta http-equiv="expires" content="0">    
	<meta http-equiv="keywords" content="${session.currentSong.getSongname() }">
    ...
    <script type="text/javascript">
		window.onload=function(){
    		...
    		if(currentU==null || currentU.equals("")){
    			$(".submitC").click(function(){
			    alert("您还没有登录，请登陆后再发言哦~")
			    return false;
			});
    		}
  		}
  ...
  <body>
    	...
    	<div class="container">
			...
			<div class="leftPart">
				<div class="row listDesc">
					<div class="col-sm-4">
						<img class="repImg" src="${session.currentSong.getSongpic() }"/>
					</div>
					<div class="col-sm-8">
						<h5>${session.currentSong.getSongname() }</h5>
						歌手：<b>${session.currentSong.getSongsinger() }</b><br>
						播放次数：${session.currentSong.getSongplaynum() }<br>
						介绍：
						<p>	
						${session.currentSong.getSongdesc() }
						</p>
						上传者：<s:if test="#session.currentSong.getUser()!=null">${session.currentSong.getUser().getNickname() }</s:if>  
						<s:else>管理员</s:else>
					</div>
				</div>
				<div class="bigPart">
					...
					<table class="table table-hover table-striped">
						<thead>
							<tr><th>&nbsp;</th><th>歌曲标题</th><th>歌手</th><th>操作</th>
							</tr>
						</thead>
						<tbody>
							<tr><td>1</td><td>${session.currentSong.getSongname() }</td><td>${session.currentSong.getSongsinger() }</td>
							<td><a href="${session.currentSong.getSongsrc() }" class="btn btn-sm btn-success" target="_blank"><i class="fui-triangle-down"></i>&nbsp;下载</a></td>
							</tr>
				...
				<div class="bigPart">
					<h4>评论区</h4>
					<form class="form" role="form" action="submitComments" method="post">
		        		 <div class="form-group row">
	        		 		<div class="col-1"></div>
					    	<textarea required="true" name="Ccontent"  class="col-10 form-control" placeholder="听完歌就跑？不说两句？"></textarea>
						  </div>
						  <!-- 判断是否登录 -->
						  <div class="form-group row">
						    <div class="col-10"></div>
						    <button type="submit" class="btn btn-success submitC"><i class="fui-bubble"></i>&nbsp;皮一下</button>
						  </div>
		        	</form>
		        	<hr>
		        	
		        	<!-- 已有评论 -->
		        	<s:iterator value="#session.currentComments" id="currentc">
			        	<div class="row commentsBox">
			        		<div class="col-sm-2 leftCom">${currentc.getUser().getNickname() }</div>
			        		<div class="col-sm-8">${currentc.getCcontent() }</div>
			        		<div class="col-sm-2">${currentc.getCtime()}</div>
			        	</div>
		        	</s:iterator>
				</div>
			</div>
	  ...
	  <script src="http://www.jq22.com/jquery/jquery-2.1.1.js"></script>
		...
		{
			"img":"${session.currentSong.getSongpic()}",
		...
		var player = new MPlayer({
			...
			lrcTopPos: 34,
			// 列表模板，用${变量名}$插入模板变量
			listFormat: '<tr><td>${session.currentSong.getSongname()}</td><td>${session.currentSong.getSongsinger()}</td><td>见控制条时长</td></tr>',
			// 音量滑块改变事件名称
